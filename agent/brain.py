...
class JobBrain:
    ...
    def _build_candidate_scoring_summary(self, query: str, ideal_text: str) -> dict:
        ...
        return {
            ...
            "commute_base": (
                f"{personal.get('location', {}).get('city', 'Base Location')}; assess commute and office days for Netherlands roles"
            ...
            "latest_experience_detail": self._compact_scoring_text(
    ...
    def _build_opportunity_scope_summary(self) -> dict:
        search_scope = dict(self.preferences.get("_runtime_search_scope") or {})
        return {
            "primary_goal": "Find professional starter opportunities that match the candidate's academic merit, IT foundation, and skills.",
            "not_perfect_match_required": True,
            "do_not_limit_to_design_roles": True,
            "favorite_domain_not_required": "The job does not need to be the candidate's absolute favorite domain if it is realistic, professional, and career-building.",
            "natural_strength_roles": self.preferences.get("target_roles", [
                "UX/UI/product/digital design",
                "brand/creative strategy",
                "digital marketing/content/e-commerce",
                "product/web operations and project coordination",
            ]),
            "broad_acceptable_domains": self.preferences.get("acceptable_domains", [
            ...
            "lower_score_but_not_auto_reject": [
                ...
                "basic administrative, receptionist, retail sales assistant, front desk, or general customer support roles (score low, below 50, since they do not build on the candidate's professional background)",
...
CV path: {self.profile.get('cv_path', f"cv/{((self.profile.get('personal') or {}).get('first_name', '') + ' ' + (self.profile.get('personal') or {}).get('last_name', '')).strip() or 'Your Name'} - CV Resume.pdf")}
